...
def _render_favorites_table(rows: pd.DataFrame, *, key_prefix: str) -> None:
    ...
    for _, row in rows.iterrows():
        ...
        favorite_notice_id = clean(row.get("_notice_id"))
        favorite_source_key = clean(row.get("_source_key")) or "iris"
        review_value = _resolve_live_favorite_value(
            favorite_source_key,
            favorite_notice_id,
            clean(row.get("Review")) or FAVORITE_REVIEW_STATUS,
        )
        favorite_inflight = _is_favorite_toggle_inflight(favorite_source_key, favorite_notice_id)
        favorite_href = build_favorite_toggle_href(
            page_key="favorites",
            notice_id=favorite_notice_id,
            current_value=review_value,
            source_key=favorite_source_key,
            notice_title=clean(row.get("Title")),
        )
        favorite_label = "저장 중" if favorite_inflight else ("해제" if review_value == FAVORITE_REVIEW_STATUS else "등록")
        favorite_class = " is-active" if review_value == FAVORITE_REVIEW_STATUS else ""
        if favorite_inflight:
        ...
        row_html.append(
            "".join(
                [
                    ...
                    (
                        f'<div class="queue-grid-cell is-center"><span class="queue-grid-favorite{favorite_class}">{favorite_label}</span></div>'
                        if favorite_inflight
                        else f'<div class="queue-grid-cell is-center"><a class="queue-grid-favorite{favorite_class}" href="{escape(favorite_href, quote=True)}" target="_self">{favorite_label}</a></div>'
                    ),
                    '</div>',
                ]
            )
...
def _render_notice_queue_table(rows: pd.DataFrame, *, key_prefix: str) -> None:
    ...
    for _, row in rows.iterrows():
        ...
        review_value = _resolve_live_favorite_value(source_key, notice_id, clean(row.get("Review")))
        favorite_inflight = _is_favorite_toggle_inflight(source_key, notice_id)
        favorite_href = build_favorite_toggle_href(
            page_key="notice_queue",
            notice_id=notice_id,
            current_value=review_value,
...
def render_notice_queue_ui_styles() -> None:
    st.markdown(
        ...
        .queue-grid-favorite.is-disabled {
          pointer-events: none;
          opacity: 0.64;
          cursor: wait;
        }
